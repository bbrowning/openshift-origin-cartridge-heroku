...
buildpack-setup() {
	...
	export STACK="cedar" # cedar-14 is too new to work with OpenShift
...
buildpack-execute() {
	if [[ -n "$BUILDPACK_URL" ]]; then
		...
		selected_name="$($selected_path/bin/detect $build_path)"
	else
		# force heroku-buildpack-multi to detect first if exists
		if ls "$buildpack_path/heroku-buildpack-multi" > /dev/null 2>&1; then
			selected_name="$($buildpack_path/heroku-buildpack-multi/bin/detect $build_path)" \
	...
	cd "$build_path"
	$selected_path/bin/compile "$build_path" "$cache_path" "$env_path"
	if [[ -f "$selected_path/bin/release" ]]; then
		$selected_path/bin/release "$build_path" "$cache_path" > "$build_path/.release"
	fi
	cd - > /dev/null
